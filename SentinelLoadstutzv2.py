...
class Sentinel2Loaderv2:
    ...
    def __init__(self, dataPath, user, password, apiUrl='https://scihub.copernicus.eu/apihub/', showProgressbars=True, dateToleranceDays=5, cloudCoverage=(0,80), deriveResolutions=True, cacheApiCalls=True, cacheTilesData=True, loglevel=logging.DEBUG, nirBand='B08', savepath = ''):
        '''
        Instantiates a Sentinel2Loaderv2 object to download satellite imagery from Sentinel-2 using Copernicus.

        Parameters:
        self(): 
        dataPath(str): A string where extraneous filenames will be saved. Recommended to lead to a trash path.
        user(str): Username for Copernicus login.
        password(str): Password for Copernicus login.
        apiUrl(str): The path to copernicus scihub.
        showProgressbars(bool): If true, shows progress bars during image downloads.
        dateToleranceDays(int): Limit on how many days away from target date image can be and still be downloaded.
        cloudCoverage(tuple): A tuple of 2 integers, denoting the range of valid cloud coverage percentages.
        deriveResolutions():
        cacheApiCalls(bool):
        cacheTilesData(bool): 
        loglevel(): 
        nirBand(str): 
        savepath(str): A string where the main imagery and savefiles will be saved. 

        Returns:
        A Sentinel2Loaderv2 object.
        '''
    ...
    def getProductBandTilesDeprecated(self, geoPolygon, bandName, resolution, dateReference, check=True):
        ...
        for index, sp in products_df.loc[selectedTiles].iterrows():
            ...
            date1 = m1.group(1)
            downloadFilename = self.dataPath + "%s%s%s.tiff" % (date1, sp['uuid'], m.group(2))

            # downloadFilename = self.dataPath + "/products/%s/%s/%s.tiff" % (date1, sp['uuid'], m.group(2)) formerly this <<<< 
            ...
            if True == True:
                ...
                #########################this is the main download #####################33

                # include a check here, if check == True:


                if count ==0:
                    if check == True:
                        print("check == true")
                        if os.path.exists(self.savepath + '%s%s%s.jp2' % (date1, sp['uuid'], bandName)):
                            fpath = self.savepath + '%s%s%s.jp2' % (date1, sp['uuid'], bandName)
                            print(fpath)
                            print("File {fpath} already saved to user data!".format(fpath=fpath))
                            logger.info("File already saved to userdata!")
                        else:
                            fpath = self.savepath + '%s%s%s.jp2' % (date1, sp['uuid'], bandName)
                            print(fpath)
                ...
                if bandName=='deprecated':# formerly TCI
    ...
    def getProductBandTiles(self, geoPolygon, bandName, resolution, dateReference, override_path = ' ', check=True):
        """
        Downloads and returns file names with Sentinel2 tiles that best fit the polygon area at the desired date reference. It will perform up/downsampling if deriveResolutions is True and the desired resolution is not available for the required band.
        
        Parameters:
        self():
        geoPolygon(Polygon): A polygon object with the target region for image download.
        bandName(str): Name of the band to be downloaded (ie. 'B04')
        resolution(str): A string of the specified resolution ('10m' or '20m'). See Sentinel-2 documentation for details.
        dateReference(date): The target date for the image download.
        override_path(str): If not default (' '), then overrides self.savepath and saves image at the given directory.
        check(bool): If check is True, checks if path exists before downloading.
        
        Returns:
        fpath(str): A path to the downloaded image band.
        ...
        footprints = [desiredRegion]
        trueindex = False
        missingfull = Polygon(geoPolygon)
        ...
        for index, pf in products_df_sorted.iterrows():
            ...
            missingfull = (missingfull.symmetric_difference(footprint)).difference(footprint)
            # if doesn't cover full area, reset full area
            if missingfull.area>0:
                missingfull = Polygon(geoPolygon)

                " I may want to add a count here, so it only saves the first full area file"
            # else mark index as a full value index
            else:
                trueindex = True
                fullindex = index
                print("full index exists!!!")
                logger.info("Full Index found at {fi}".format(fi=fullindex))
                missingfull = Polygon(geoPolygon)
        if trueindex == True:
            selectedTiles = []
            selectedTiles.append(fullindex)
        
                # this is where I would save a tile that covers the entire desired area!!!!!!!!
        if missing.area>0:
            raise Exception('Could not find tiles for the whole selected area at date range')
        ...
        for index, sp in products_df.loc[selectedTiles].iterrows():
            ...
            if True == True:
                ...
                if check == True:
                    print("check == true")
                    if os.path.exists(spath + '%s%s%s.jp2' % (date1, sp['uuid'], bandName)):
                        fpath = spath + '%s%s%s.jp2' % (date1, sp['uuid'], bandName)
                        print(fpath)
                        print("File {fpath} already saved to user data!".format(fpath=fpath))
                        logger.info("File already saved to userdata!")
                    else:
                        fpath = spath + '%s%s%s.jp2' % (date1, sp['uuid'], bandName)
                        print(fpath)
                        downloadFile(url, spath + '%s%s%s.jp2' % (date1, sp['uuid'], bandName) , self.user, self.password)
                else:
                    fpath = spath + '%s%s%s.jp2' % (date1, sp['uuid'], bandName)
                    print(fpath)
                    downloadFile(url, spath + '%s%s%s.jp2' % (date1, sp['uuid'], bandName) , self.user, self.password)
            ...
            tileFiles.append(fpath)

        return fpath


    # new idea is to iterate through each image, and simply paste into one very large jp2. This will result in overlap (the last image placed will be the result), and then at the end you crop.
